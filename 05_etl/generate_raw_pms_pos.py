...
Now with explicit source-system variation per branch/POS/ERP.
"""
...
def source_for_property(pid: str) -> str:
    for sys_name, cfg in P.SOURCE_SYSTEMS.items():
        if not sys_name.startswith("PMS"):
            continue
        if pid in cfg.get("properties", []):
            return sys_name
    return "PMS-HOTEL-01"


def source_for_outlet(oid: str) -> str:
    for sys_name, cfg in P.SOURCE_SYSTEMS.items():
        if not sys_name.startswith("POS"):
            continue
        if oid in cfg.get("outlets", []):
            return sys_name
    return "POS-FNB-01"
...
def generate_pms() -> pd.DataFrame:
    ...
    for d in P.daterange():
        ...
        for pid, name, city, region, rooms, cls, base_occ, base_adr in P.PROPERTIES:
            ...
            if n == 0:
                continue
            system = source_for_property(pid)
            for _ in range(n):
                seq += 1
                seg = r.choice(P.SEGMENTS, p=list(P.SEGMENT_WEIGHT.values()))
                ch = r.choice(P.CHANNELS, p=list(P.CHANNEL_WEIGHT.values()))
                rt, rt_name, rt_mult = P.ROOM_TYPES[r.choice(len(P.ROOM_TYPES), p=P.ROOM_TYPE_WEIGHT)]
                ...
                pay = r.choice(["Paid", "Unpaid", "Partial"], p=[0.78, 0.12, 0.10]) if status == "Confirmed" else "Unpaid"
                ...
                rows.append(dict(
                    ...
                    booking_date=fmt_date_for(d, r, system),
                    check_in=fmt_date_for(ci, r, system),
                    check_out=fmt_date_for(co, r, system),
                    ...
                    room_rate=fmt_num_for(rate, r, system, 0),
                    room_revenue=fmt_num_for(rev, r, system, 0),
    ...
    df["prop_code"] = ""  # stray blank column
        ...
        for oid, oname, pid, bu, otype, covers in P.OUTLETS:
            ...
            for _ in range(n_tx):
                ...
                for ln in range(1, n_lines + 1):
                    ...
                    rows.append(dict(
                        transaction_id=tid,
                        transaction_date=fmt_date_for(d, r, system),
                        ...
                        category=cat_out,
                        quantity=qty,
                        gross_sales=fmt_num_for(gross, r, system),
                        discount="" if (disc == 0 and maybe_null(r)) else fmt_num_for(disc, r, system),
                        net_sales=fmt_num_for(net, r, system),
...
def main():
    ...
    df_pms = generate_pms()
    df_pos = generate_pos()
    df_pms.to_csv(os.path.join(OUT_PMS, "pms_bookings_raw.csv"), index=False)
    df_pos.to_csv(os.path.join(OUT_POS, "pos_transactions_raw.csv"), index=False)
    print(f"[gen] PMS {len(df_pms):,} rows, POS {len(df_pos):,} rows")
